...
import { convertDate, reverseDate } from "@/lib/utils";
...
export default function NewsEventsTable({ newsEvents }: NewsEventsTableProps) {
  return (
    <section className="flex flex-col">
      <table className="bg-white/[3%] mb-2 rounded-md shadow-[0_7px_7px_rgba(2,2,2,1)] border-collapse outline outline-1 outline-white/20 table-fixed">
        <thead>
      ...
      <p className="text-sm text-white/75 self-end">
        ...
        <Link href="https://www.cryptocraft.com/calendar" target="_blank">
          <span className="text-green-yellow hover:text-green-yellow/75">CryptoCraft</span>
        </Link>
      </p>
    </section>
  );
